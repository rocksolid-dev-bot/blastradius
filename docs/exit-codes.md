...
| `2` | Usage error: an unknown flag, an invalid `--fail-on` value (anything other than `review` or `urgent`), or no `package.json`/npm lockfile found in the target directory. |
